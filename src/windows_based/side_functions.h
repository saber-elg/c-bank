...
char* create_num_password() {
    ...
    while ((ch = getch()) != 13 && i<MAX_PASSWORD_LENGTH) {
        ...
        *(password+i) = ch;
        if (ch == 8){
            if( i > 0) {  // Backspace (ASCII backspace==8)
            putch('\b');  // Move cursor back, print space, move cursor back again
            putch(' ');
            putch('\b');
            i--;
            }
...
// Risplays the client related info: acount number, full name, CIN, email.
void display_client_profile(Client *client){
    system("cls");
    if (client==NULL)
    {
...
// fgets equivalance without new linw reture character '\n' the buffer should be dynamicaly allocated 
void fgets_no_newline_return(char* buffer,int size){
